...
import { movieNames, movieGenre, moviePoster, movieDescription, reviewBodies } from './randomData';
...
const makeReview = (db, movie, i) => {
  return db.collections.get('reviews').prepareCreate((review) => {
    review.body = reviewBodies[i % reviewBodies.length] || `review#${review.id}`;
    review.movie.set(movie);
  });
};

const makeReviews = (db, movie, count) => times((i) => makeReview(db, movie, i), count);
...
const generate = async (db, movieCount, reviewsPerPost) => {
  ...
  const reviews = flatMap((movie) => makeReviews(db, movie, fuzzCount(reviewsPerPost)), movies);
